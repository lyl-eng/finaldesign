...
class TranslationRefinementAgent(BaseAgent):
    ...
    def __init__(self, config=None):
        ...
        # 🔥 截断处理模式配置
        # "retry" - 检测到截断后重新翻译（默认）
        # "accept" - 检测到截断后直接使用当前结果，不重试
        self.truncation_mode = "accept"  # 当前设置为模式2：接受截断结果
    ...
    def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 🔥 显示截断处理模式
        mode_desc = "接受截断结果，不重试" if self.truncation_mode == "accept" else "重新翻译截断内容"
        self.info(f"截断处理模式：【{mode_desc}】")
                    ...
                    if result and result.get("success"):
                        ...
                        if progress_callback:
                            progress_callback(
                                ...
                                total_units, 
                                "translation", 
                                f"已翻译 {completed_units}/{total_units} 个单元 (批次 {chunk_idx}/{total_chunks})"
                            )
    ...
    def _translate_chunk(self, chunk: List[CacheItem], context_chunk: List[CacheItem], 
        ...
        try:
            ...
            if not translated_texts:
                ...
                for i, (src, trans) in enumerate(zip(source_texts[:chunk_size], translated_texts[:chunk_size])):
                    ...
                    # 🔥 特殊检测：参考文献（调整阈值）
                        ...
                        # 参考文献由于大量保留原文（作者、年份、DOI等），译文通常是原文的40-60%
                        # 只有译文<40%才认为可能截断
                        if len(src) > 1000 and len(trans) < len(src) * 0.4:
                            problem_indices.append((i, f"参考文献可能截断({len(trans)}/{len(src)})"))
                        # 或者译文<20%，肯定是截断
                        elif len(src) > 100 and len(trans) < len(src) * 0.2:
                            problem_indices.append((i, f"参考文献严重截断({len(trans)}/{len(src)})")) 
                ...
                if problem_indices:
                    ...
                    if self.truncation_mode == "accept":
                        # 模式2：接受截断结果，不重试
                        self.warning(f"  ⚠ 检测到 {len(problem_indices)} 行可能被截断，但当前模式为【接受截断】，不进行重试")
                        for i, reason in problem_indices:
                            src_len = len(source_texts[i])
                            trans_len = len(translated_texts[i])
                            self.warning(f"    → 行{i+1}: {reason} - 接受当前结果 (原文:{src_len}字 → 译文:{trans_len}字)")
                    else:
                        # 模式1：重试截断的行
                        ...
                        for i, reason in problem_indices:
                            ...
                            if single_translation and single_translation.strip():
                                ...
                                if is_reference:
                                    # 参考文献由于保留原文（作者、年份、DOI等），译文通常是原文的40-60%
                                    # 只有真正异常时才警告
                                    if src_len > 1000 and trans_len < src_len * 0.4:
                                        self.warning(f"    ⚠ 第 {i+1} 行参考文献可能被截断 ({trans_len}/{src_len}，预期约{int(src_len*0.5)}字符)")
                                    elif src_len > 100 and trans_len < src_len * 0.2:
                                        self.warning(f"    ⚠ 第 {i+1} 行参考文献严重截断 ({trans_len}/{src_len})")
                                elif src_len > 100 and trans_len < src_len * 0.4:
                                    self.warning(f"    ⚠ 第 {i+1} 行单独翻译也被截断 ({trans_len}/{src_len})")
                                
                                translated_texts[i] = single_translation
                                self.info(f"    ✓ 第 {i+1} 行翻译完成 (长度: {trans_len})")
                            else:
                                # 如果单独翻译也失败，使用原文标记
            ...
            self.info(f"  ✓ 实体一致性检查完成: {len(translated_texts)} 行")
...
（在这里输出带序号的最佳译文）
    ...
    def _batch_tear_verification(self, source_texts: List[str], translated_texts: List[str],
        ...
        返回: (verified_texts, back_translations, quality_scores)
        """
        # ===== 批量回译 =====
        self.info(f"    → 批量回译...")
