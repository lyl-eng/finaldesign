...
from UserInterface.EditView.AgentFlow.AgentFlowPage import AgentFlowPage
...
class BottomCommandBar(Base,CardWidget):
    ...
    def command_play(self) -> None:
        ...
        use_multi_agent = config.get("use_multi_agent_mode", False)
...
class EditViewPage(Base,QFrame):
    ...
    def __init__(self, text: str, window, plugin_manager, cache_manager, file_reader) -> None:
        ...
        # Agent流程展示页面设置
        self.agent_flow_page = AgentFlowPage()

        # 向堆叠控件添加页面，即信息展示页面、监控页面与Agent流程页面
        self.stacked_widget.addWidget(self.main_page)
        self.stacked_widget.addWidget(self.monitoring_page)
        self.stacked_widget.addWidget(self.agent_flow_page)
    ...
    def toggle_page(self):
        """
        切换页面：main_page(0) → monitoring_page(1) → agent_flow_page(2) → main_page(0)
        """
        current_index = self.stacked_widget.currentIndex()
        ...
        # 更新箭头按钮图标方向（可选）
        if new_index == 0:
            self.bottom_bar_main.arrow_btn.setIcon(FIF.UP)
        elif new_index == 1:
            self.bottom_bar_main.arrow_btn.setIcon(FIF.DOWN)
        else:
            self.bottom_bar_main.arrow_btn.setIcon(FIF.SYNC)
